...
class JsonPresenter:
    ...
    def convert_to_ndarray_rgb(
        ...
        for shape in json_dict["shapes"]:
            ...
            try:
                # 尝试从id_list中查找标签索引
                label_index = id_list.index(shape["label"])
                color = colors[label_index % len(colors)]  # 使用模运算防止索引越界
            except ValueError:
                # 如果标签不在id_list中，使用标签的哈希值来生成一个稳定的颜色
                label_hash = hash(shape["label"]) % len(colors)
    ...
    def convert_to_ndarray_gray(
        ...
        for shape in json_dict["shapes"]:
            ...
            try:
                ...
                # 如果标签不在id_list中，使用默认ID（0，表示背景）
                id = 0
            if thickness == -1:  # 填充多边形
                cv2.fillPoly(image_array, [points], id)
            else:  # 绘制多边形轮廓
                cv2.polylines(
                    image_array,
                    [points],
